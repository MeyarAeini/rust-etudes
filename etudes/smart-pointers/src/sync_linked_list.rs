...
impl<T> LinkedList<T> {
    ...
    pub fn push_back(&mut self, value: T) {
        ...
        match self.back.take() {
            Some(back) => {
                if let Ok(mut back) = back.lock() {
                    back.prev = Some(Arc::clone(&head));
                }
            ...
            None => {
                self.front = Some(Arc::clone(&head));
                self.back = Some(head);
            }
        }
    }
    ...
    pub fn push_front(&mut self, value: T) {
        ...
        match self.front.take() {
            Some(front) => {
                if let Ok(mut front) = front.lock() {
                    front.next = Some(Arc::clone(&tail));
                }
                if let Ok(mut tail) = tail.lock() {
                    tail.prev = Some(front);
                }
                self.front = Some(tail);
            }
            None => {
                self.back = Some(Arc::clone(&tail));
                self.back = Some(tail);
            }
        }
    }
    ...
    pub fn pop_back(&mut self) -> Option<T> {
        let back_node = self.back.take()?;
        //this is not working
        if let Ok(mutex) = Arc::try_unwrap(back_node) {
            let mut back_guard = mutex.into_inner().ok()?;
            let result = Some(back_guard.value);

            if let Some(next_node) = back_guard.next.take() {
                if let Ok(mut next_node) = next_node.lock() {
                    next_node.prev = None;
                }
                self.back = Some(next_node);
            } else {
                self.front = None;
            }
    ...
    //        Arc::try_unwrap(link) //return the inner value of Rc if it only has one strong reference
